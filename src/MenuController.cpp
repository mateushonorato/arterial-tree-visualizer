...
void MenuController::render(AnimationController& animCtrl, ArterialTree& tree, TreeRenderer& renderer, bool hideMainPanel) {
    if (!hideMainPanel) {
        ...
        if (ImGui::CollapsingHeader("Animação", ImGuiTreeNodeFlags_DefaultOpen)) {
            ...
            if (animChanged) animCtrl.m_visualDirty = true;
        }
        ...
        if (ImGui::CollapsingHeader("Ajustes Visuais", ImGuiTreeNodeFlags_DefaultOpen)) {
            ...
            if (visualChanged) animCtrl.m_visualDirty = true;
        }
        ...
        if (animCtrl.getCurrentMode() != AnimationController::ModeWireframe) {
            if (ImGui::CollapsingHeader("Iluminação", ImGuiTreeNodeFlags_DefaultOpen)) {
                ...
                if (ImGui::RadioButton("Phong", animCtrl.lightingMode == 0)) { animCtrl.lightingMode = 0; lightChanged = true; }
                ImGui::SameLine();
                if (ImGui::RadioButton("Gouraud", animCtrl.lightingMode == 1)) { animCtrl.lightingMode = 1; lightChanged = true; }
                ImGui::SameLine();
                if (ImGui::RadioButton("Flat", animCtrl.lightingMode == 2)) { animCtrl.lightingMode = 2; lightChanged = true; }
                ...
                if (lightChanged) animCtrl.m_visualDirty = true;
            }
        }
        ...
        if (ImGui::CollapsingHeader("Ferramentas de Corte", ImGuiTreeNodeFlags_DefaultOpen)) {
            ...
            if (clipChanged) animCtrl.m_visualDirty = true;
        }
    ...
    if (selIdx != -1 && selIdx < (int)tree.segments.size()) {
        ...
        if (ImGui::IsItemHovered()) ImGui::SetTooltip("Resistência Geométrica (L / r^4).\nUnidade: mm^-3");
